...
head(d, 5000)
frequent_words_and_frequency = head(d, 8000)
...
```{r}
example = read.table("200-300.txt")
example = unlist( example, use.names=F)

extra_words = setdiff(example, new_words)

columnNames = colnames(ReviewWordMatrix)

to_be_calc = setdiff(extra_words, columnNames)
```
...
vector = c("\\$", "shit hole", "low standard", "\\!", "\\.", "10/10", "\\?", "stay away" ,":\\(", ":\\)")


for (i in 1:nrow(yelp_train)){
  for (j in 1:length(vector)){
      yelp_train[i,vector[j]] = str_count(yelp_train$text[i], regex(vector[j], ignore_case=T)) # ignore the upper/lower case in the text
  }
}
...
```{r}
finalist = setdiff (extra_words, to_be_calc)

new_train1 = ReviewWordMatrix[,new_words]
new_train2 = ReviewWordMatrix[,finalist]


final_new_train1=as.data.frame(as.matrix(new_train1))
final_new_train2=as.data.frame(as.matrix(new_train2))

temp_df = cbind(final_new_train1, final_new_train2)

final_df = cbind(yelp_train, temp_df)
...
lasso_cv = cv.glmnet(Xmat, Ymat, alpha = 1, nfold=5)
plot(lasso_cv)
```
...
for (i in 1:nrow(yelp_2combined)){
  for (j in 1:length(vector)){
      yelp_2combined[i,vector[j]] = str_count(yelp_2combined$text[i], regex(vector[j], ignore_case=T)) # ignore the upper/lower case in the text
  }
}
...
finalist = setdiff (extra_words, to_be_calc)
library(tm)
removeWords(finalist, "lowlight")
removeWords(finalist, "untidy")
finalist
new_test1 = ReviewWordMatrix2[,new_words]
new_test2 = ReviewWordMatrix2[,finalist]


final_new_test1=as.data.frame(as.matrix(new_test1))
final_new_test2=as.data.frame(as.matrix(new_test2))

temp_df = cbind(final_new_test1, final_new_test2)

temp_df$newID = c(seq(1,nrow(yelp_2combined),1))

final_df2 <- cbind(yelp_2combined, temp_df)
...
tempfinal <- as.matrix(final_df2[,-c(1:4, 8, 12, 1936)])
```


```{r}
star_out = data.frame(Id=final_df2$Id, Expected=predict(lasso_cv, newx = tempfinal, s = "lambda.min"))
write.csv(star_out, file='14_Submission_lasso.csv', row.names=FALSE)
```
...
write.csv(submit, file='14_Submission_lasso_submit.csv', row.names=FALSE)
```
