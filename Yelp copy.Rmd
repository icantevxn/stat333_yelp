...
frequent_words_and_frequency = head(d, 500)
...
library(stringr)
new_words = c("is", "not", "good", "great", "nice", "well", "best", "friendly", "never", "amazing", "excellent", "bad", "long")
new_X = matrix(0, nrow(yelp_train), length(new_words))
colnames(new_X) = new_words
for (i in 1:length(new_words)){
  new_X[,i] = str_count(yelp_train$text, regex(new_words[i], ignore_case=T)) # ignore the upper/lower case in the text
}
...
```{r new words pvalue}
# testing if a specific word count is associated with star rating
new_pvals_new = rep(0,14)
names(new_pvals_new) = new_words
for (i in 1:14){
...
for (i in 1:nrow(yelp_train)){
  for (j in 1:length(new_words)){
      yelp_train[i,new_words[j]] = str_count(yelp_train$text[i], regex(new_words[j], ignore_case=T)) # ignore the upper/lower case in the text
  }
}
...
```{r test using train}
dat = yelp_train[,-c(1,3:5,9,13, 21)]
colnames(dat)
